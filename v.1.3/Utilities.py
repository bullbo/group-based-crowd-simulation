...
# NOT USED
def turn_vector_to_point(v1, point, gaze_angle):
    """ Turns a vector v1, so that its field of view defined by the gaze_angle includes a point. """
    fov1, fov2 = rot_v(v1, -gaze_angle), rot_v(v1, gaze_angle)
    if check_vector_inbetween(fov1, fov2, point) == False:
        theta = -min(angle2(fov1, point), angle2(fov2, point), key=abs)
        output = rot_v(v1, np.degrees(theta/2))
    else:
        output = v1
    return output