...
class SF_Model:
    ...
    def step(self, dt):
        ...
        for agent in self.agents:
            ...
            goal_direction = normalize(agent.destination - agent.position)
            ...
            acceleration = goal_force+agent_repulsion+group_attraction_force+wall_repulsion
            ...
            if type(initial_velocity) == int:
                continue
            else:
                ang = calc_angle(agent.velocity, initial_velocity)
                weight = np.exp(-(24*ang)**2)
                new_d = normalize(normalize(initial_velocity)+(1-weight)*0.1*goal_direction+weight*(agent.velocity))
                agent.velocity = weight*np.linalg.norm(agent.velocity)*new_d


            # Stop the agent if the destination is reached
            if np.all(np.isclose(agent.position, agent.destination, atol=((len(agent.group.agents)-1)*AGENT_RADIUS))): #0.1 rtol is standard
                agent.velocity = 0
                acceleration = 0
